...
export default function MessageBubble({ message }: MessageBubbleProps) {
  const renderContent = () => {
    ...
    return (
      <p className="text-sm whitespace-pre-wrap text-gray-700 font-medium">
  ...
  return (
    <div className={`flex ${message.isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`${
          message.isUser
            ? "bg-gray-100 text-gray-900 max-w-[85%] sm:max-w-md rounded-2xl"
            : "text-gray-900 max-w-[95%] sm:max-w-2xl lg:max-w-3xl xl:max-w-4xl"
        } px-3 sm:px-4 py-3 ${
          !message.isUser
            ? "animate-silver-shimmer animate-message-fade-in rounded-lg"
            : ""
        }`}
      >
