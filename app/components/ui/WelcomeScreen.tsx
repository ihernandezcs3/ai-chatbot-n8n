...
export default function WelcomeScreen({
  ...
  const displayName = userData?.tokenPayload
    ? TokenService.getUserDisplayName(userData.tokenPayload)
    : "Usuario";
  return (
    <motion.div
      key="welcome"
    ...
    >
      ...
      <div className="w-24 h-24 rounded-full bg-indigo-100 flex items-center justify-center mb-4 overflow-hidden">
        <Image
          src="/ia_circle.png"
          alt="Avatar Asistente Clave"
          width={96}
          height={96}
          className="object-cover object-top w-24 h-24"
        />
      </div>
      <div className="text-center mb-6 px-4">
        <p className="text-base font-normal text-gray-700">
          ...
          ,<br />
          Soy{" "}
          <span className="font-semibold bg-gradient-to-r from-[#9379E3] via-[#5E70D2] to-[#3A4CB1] bg-clip-text text-transparent">
            Asistente Clave
          </span>
          , tu guía y asistente en <br /> nuestra aplicación. ¿En qué puedo
          ayudarte hoy?
        </p>
      </div>
      <button
